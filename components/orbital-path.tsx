...
export default function OrbitalPath({
  ...
  const pathRef = useRef<THREE.Line>(null) as React.RefObject<THREE.Line>
  ...
  useFrame(({ clock }) => {
    if (pathRef.current && animate) {
      ...
      if (materialRef.current) {
        (materialRef.current as THREE.LineDashedMaterial & { dashOffset: number }).dashOffset -= animationSpeed * 0.01
      }
    }
  })
