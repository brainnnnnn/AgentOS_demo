    ...
    <div ref={containerRef} className="relative w-[200px] h-[200px] flex items-center justify-center">
      ...
      {orbitalEffects.map((effect, index) => (
        <div
          ...
          style={{
            ...
            borderStyle: "dashed",
            // @ts-expect-error: using SVG-style dasharray for visual effect
      ...
      {[...Array(8)].map((_, i) => (
        <motion.div
          ...
          style={{
            width: `${pseudoRandom(i) * 2 + 1}px`,
            height: `${pseudoRandom(i + 100) * 2 + 1}px`,
            boxShadow: "0 0 2px rgba(255, 255, 255, 0.8)",
          }}
          initial={{
            x: pseudoRandom(i + 200) * 200 - 100,
            y: pseudoRandom(i + 300) * 200 - 100,
            opacity: 0,
          }}
          animate={{
            x: pseudoRandom(i + 400) * 200 - 100,
            y: pseudoRandom(i + 500) * 200 - 100,
            opacity: [0, 0.8, 0],
          }}
          transition={{
            duration: pseudoRandom(i + 600) * 3 + 2,
            repeat: Number.POSITIVE_INFINITY,
            ease: "easeInOut",
            delay: pseudoRandom(i + 700) * 2,
          }}
        />
      ))}
    </div>
