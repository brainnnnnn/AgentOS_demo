...
// ContentPanel 组件的 props 接口添加 useDefaultUIColor
interface ContentPanelProps {
  ...
  sidebarWidth?: number
  theme?: Theme
  useDefaultUIColor?: boolean // 添加默认 UI 颜色使用标志
}

// 添加默认 UI 颜色常量 (Cosmic Blue 主题颜色)
const defaultUIColor = "#FFFFFF"

// ContentPanel 函数参数添加 useDefaultUIColor
export default function ContentPanel({
  ...
  sidebarWidth = 0,
  theme = themes[0],
  useDefaultUIColor = true, // 默认设为 true
}: ContentPanelProps) {
  ...
  // 门票弹窗(1号弹窗)的边距值 - 加宽 15%
  const ticketMargin = "29px"

  // 演讲者弹窗(2号弹窗)的边距值 - 加宽 40%
  const speakerMargin = "35px"

  // 闪电演讲弹窗(4号弹窗)的边距值 - 加宽 40%
  ...
  // 添加用于检测语言变化的 useEffect
  useEffect(() => {
    // 即使语言变化也保持当前弹窗状态
    // 此 useEffect 仅在语言变化时执行
  ...
  // Calculate modal position based on sidebar state - 메인 内容 기준 가운데 정렬
  const getModalStyle = () => {
    return {
  ...
  useEffect(() => {
    const checkScroll = () => {
      ...
      // 检查是否有滚动 - 更精确的计算
      const hasVerticalScroll = content.scrollHeight > content.clientHeight + 10 // 增加余量值
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      // 初始设置 hasScrolled 为 false 以显示提示
          ...
          >
            ...
            <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
              ...
              <motion.div
                ...
                style={{
                  backgroundImage: `repeating-linear-gradient(0deg, transparent, transparent 5px, ${accentColor} 5px, ${accentColor} 6px)`,
            ...
            >
              ...
              {/* 스크롤 그라디언트 인디케이터 - 스크롤이 있고 맨 아래가 아니고 아직 스크롤하지 않았을 때만 표시 */}
              {hasScroll && !isScrolledToBottom && !hasScrolled && (
                <motion.div
                  className="absolute left-0 right-0 bottom-0 h-24 pointer-events-none transition-opacity duration-300"
                  style={{
                ...
                >
                  <div className="absolute left-1/2 bottom-6 transform -translate-x-1/2 flex flex-col items-center">
                    <div className="text-white/80 text-xs mb-2">스크롤하여 더 보기</div>
                    <motion.div
                      animate={{ y: [0, 8, 0] }}
